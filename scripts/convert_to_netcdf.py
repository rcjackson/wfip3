...
"""
input_path = '/lcrc/group/earthscience/rjackson/wfip3/caco/lidar'
...
def hpl2dict(file_path):
    ...
    for ri in range(0, rays_n):  # loop rays
        lines_temp = lines[
            header_n + (ri * gates_n) + ri + 1 : header_n
            + (ri * gates_n)
            + gates_n
            + ri
            + 1
...
def hpl_to_netcdf(file_path, path_out, institution=None, contact=None, overwrite=False,
    ...
    dataset_temp.history = "File created on %s " % datetime.datetime.now().strftime(
        "%d %b %Y %H:%M"
    )

    decimal_time = dataset_temp.createVariable(
        "decimal_time", np.float64, ("NUMBER_OF_RAYS")
    )
    ...
    rv = dataset_temp.createVariable(
        "radial_velocity", np.float32, ("NUMBER_OF_GATES", "NUMBER_OF_RAYS")
    )
    rv.units = "m s-1"
    rv.long_name = "Doppler velocity along line of sight"
    rv.correction_factor = correction_factor
    rv[:, :] = data_temp["radial_velocity"] + correction_factor

    intensity = dataset_temp.createVariable(
        "intensity", np.float32, ("NUMBER_OF_GATES", "NUMBER_OF_RAYS")
    )
    intensity.units = "unitless"
    intensity.long_name = "SNR + 1"
    intensity[:, :] = data_temp["intensity"]

    beta = dataset_temp.createVariable(
        "beta", np.float32, ("NUMBER_OF_GATES", "NUMBER_OF_RAYS")
    )
    beta.units = "m-1 sr-1"
    beta.long_name = "attenuated backscatter"
    beta[:, :] = data_temp["beta"]

    gate_centers = dataset_temp.createVariable(
        "gate_centers", np.float32, ("NUMBER_OF_GATES")
    )
...
def to_netcdf_l1(file_path, file_name_out, lidar_info, path_out):
    ...
    ds_temp.azimuth.attrs["comment"] = (
        "corrected azimuth angle (az_corrected=az_measured-bearing) --> az_corrected = 0 deg points to geographical North"
    )
    if lidar_info.bearing != 0:
        ds_temp.attrs["description"] = (
            "corrected data of Halo Photonics Streamline, corrected variables: azimuth"
        )
    elif lidar_info.gc_corr != 0 and lidar_info.bearing != 0:
        ds_temp.attrs["description"] = (
            "corrected data of Halo Photonics Streamline, corrected variables: azimuth, gate_centers"
        )
    elif lidar_info.gc_corr == 0 and lidar_info.bearing == 0:
        ds_temp.attrs["description"] = (
            "corrected data of Halo Photonics Streamline, corrected variables: none"
        )
    ...
    dn_time_temp_var = xr.Variable(
        ["NUMBER_OF_RAYS"],
        dn_time_temp,
        attrs={
            "units": "Days since 01-01-0001 00:00:00",
            "long_name": "start time number of each ray",
        },
    )
    ds_temp = ds_temp.assign(datenum_time=dn_time_temp_var)

    dt_time_temp_var = xr.Variable(
        ["NUMBER_OF_RAYS"],
        unix_time,
        attrs={
            "units": "Seconds since 01-01-1970 00:00:00",
            "long_name": "start time number of each ray",
