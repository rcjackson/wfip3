...
def Rain_Par(state,Z,LWC,RR,Nw,Dm,NewM,D,N_da,NdE,he,w,Pia):
    ...
    for m in range(np.size(state)):
        if state[m]==10 or state[m]==5:#rain case
            ...
            w_temp = w[m] * np.ones_like(LastN)
            value3=np.nansum(np.prod([np.power(D[m],3),LastN,dif2,w_temp],axis=0))
...
Root="/lcrc/group/earthscience/rjackson/wfip3/barg/mrr_raw/"
os.chdir(Root)
...
##    print('altures',HIcolum2)
    ...
    dataset.description='MRR radar processed by RaProM'
    dataset.author='Robert Jackson, Paytsar Muradyan'
    dataset.raprom_doi="10.3390/rs12244113"
    ...
    dataset.createDimension('range',len(HIcolum2[1:]))
    ...
    nc_Format_times=dataset.createVariable('time', 'float64', ('time',))
    nc_ranges_H=dataset.createVariable('range','f',('range',))
    ...
    nc_Format_times.units='seconds since 1970-01-01T00:00:00'
    nc_Format_times.calendar='standard'
    nc_Format_times.decription='Time in UTC'

    nc_ranges_H.units = 'm'
    if np.isnan(h0_opt):
        nc_ranges_H.long_name = 'range [m AGL]'
    else:
        nc_ranges_H.long_name = 'range [m ASL]'

    nc_ranges_H_PIA.units = 'm'
    if np.isnan(h0_opt):
        nc_ranges_H_PIA.long_name = 'Height a.g.l.'
    else:
        nc_ranges_H_PIA.long_name = 'Height a.s.l.'
    

    nc_ranges_H_BB.units = 'm'
    if np.isnan(h0_opt):
        nc_ranges_H_BB.long_name = 'Height a.g.l.'
    else:
        nc_ranges_H_BB.long_name = 'Height a.s.l.'
    



    nc_ranges_DropSize.units = 'mm'
    nc_ranges_DropSize.long_name = 'Size of the water drops'
...
##        Harray=np.fromiter(Hcolum,dtype=int)
        ...
        if (dat-timeVec)>=IntTime:
            ...
            ncShape3D = ('time','range','DropSize',)
            ncShapeSweep = ('sweep', 'string_length')
            if Timecount==0:
                ##################create netcdf############
                nc_w=dataset.createVariable('W','f',ncShape2D)
                nc_w.long_name='Fall speed with aliasing correction'
                nc_w.units='m/s'
                nc_latitude = dataset.createVariable('latitude', 'f', ())
                nc_latitude.long_name = "Barge Site latitude"
                nc_latitude.units = "Degrees"
                nc_latitude[:] = 40.9
                
                nc_longitude = dataset.createVariable('longitude', 'f', ())
                nc_longitude.long_name = "Barge Sitelongitude"
                nc_longitude.units = "Degrees"
                nc_longitude[:] = -70.7833
                
                nc_altitude = dataset.createVariable('altitude', 'f', ())
                nc_altitude.long_name = "Barge Site altitude [ASL]"
                nc_altitude.units = "m"
                nc_altitude[:] = 0.
                
                sweep_number = dataset.createVariable('sweep_number', 'i', ('sweep'))
                sweep_number.long_name = "Sweep number"
                sweep_number[:] = np.array([0])
                
                sweep_mode = dataset.createVariable('sweep_mode', "S1", ('sweep', 'string_length'))
                sweep_mode.long_name = "Sweep number"
                sweep_mode[:] = np.array(["vertical_pointing".ljust(32),], dtype="S1")
                
                fixed_angle = dataset.createVariable('fixed_angle', 'f', ('sweep'))
                fixed_angle.long_name = "Fixed angle"
                fixed_angle[:] = np.array([90.])
                
                azimuth = dataset.createVariable('azimuth', 'f', ('time'))
                azimuth.long_name = "Azimuth angle"
                azimuth.units = "degrees"
                
                elevation = dataset.createVariable('elevation', 'f', ('time'))
                elevation.long_name = "Azimuth angle"
                elevation.units = "degrees"
                
                sweep_start_ray_index = dataset.createVariable('sweep_start_ray_index', 'i', ('sweep'))
                sweep_start_ray_index.long_name = "Sweep start ray index"
                sweep_start_ray_index[:] = np.array([0])
                
                sweep_end_ray_index = dataset.createVariable('sweep_end_ray_index', 'i', ('sweep'))
                sweep_end_ray_index.long_name = "Sweep end ray index"
                
                
                nc_sig=dataset.createVariable('spectral_width','f',ncShape2D)
                nc_sig.long_name='Spectral width of the dealiased velocity distribution'
                nc_sig.units='m/s'
                
                nc_sk=dataset.createVariable('Skewness','f',ncShape2D)
                nc_sk.long_name='Skewness of the spectral reflectivity with dealiasing'
                nc_sk.units='none'

                nc_kur=dataset.createVariable('Kurtosis','f',ncShape2D)
                nc_kur.long_name='Kurtosis of the spectral reflectivity with dealiasing'
                nc_kur.units='none'

                nc_PIA=dataset.createVariable('PIA','f',ncShape2D_PIA)
                nc_PIA.long_name='Path Integrated Attenuation calculated using only liquid hydrometeors according to hydrometeor type classification'
                nc_PIA.units='dB'

                nc_PIA_all=dataset.createVariable('PIA_all','f',ncShape2D_PIA)
                nc_PIA_all.long_name='Path Integrated Attenuation calculated assuming all hydrometeors are in liquid phase regardless of hydrometeor type classification'
                nc_PIA_all.units='dB'

                #nc_state=dataset.createVariable('Type','f',ncShape2D)
                #nc_state.long_name='Predominant hydrometeor type numerical value where possible values are: -20 (hail), -15 (graupel), -10 (snow), 0 (mixed), 5 (drizzle), 10 (rain) and 20 (unknown precipitation)'
                #nc_state.units=''
                
                nc_LWC=dataset.createVariable('LWC','f',ncShape2D)
                nc_LWC.long_name='Liquid Water Content calculated using only liquid hydrometeors according to hydrometeor type classification'
                nc_LWC.units='g m-3'
                
                nc_RR=dataset.createVariable('RR','f',ncShape2D)
                nc_RR.long_name='Rain Rate calculated using only liquid hydrometeors according to hydrometeor type classification'
                nc_RR.units='mm hr-1'

                nc_SnowR=dataset.createVariable('SR','f',ncShape2D)
                nc_SnowR.long_name='Snow Rate'
                nc_SnowR.units='mm hr-1'
                
                nc_Z_DA=dataset.createVariable('Z','f',ncShape2D)
                nc_Z_DA.long_name='Radar reflectivity calculated using only liquid hydrometeors according to hydrometeor type classification'
                nc_Z_DA.units='dBZ'

                nc_Z_da=dataset.createVariable('Za','f',ncShape2D)
                nc_Z_da.long_name='Attenuated radar reflectivity calculated using only liquid hydrometeors according to hydrometeor type classification'
                nc_Z_da.units='dBZ'

                nc_Z_e=dataset.createVariable('Ze','f',ncShape2D)
                nc_Z_e.long_name='Equivalent radar reflectivity'
                nc_Z_e.units='dBZ'

                nc_N_da=dataset.createVariable('Nd_liquid','f',ncShape2D)
                nc_N_da.long_name='Drop Size Distribution calculated using only liquid hydrometeors according to hydrometeor type classification'
                nc_N_da.units='log10(m-3 mm-1)'

                nc_N_daTH=dataset.createVariable('Nd','f',ncShape3D)
                nc_N_daTH.long_name='Drop Size Distribution in function of time and height calculated using only liquid hydrometeors according to hydrometeor type classification'
                nc_N_daTH.units='log10(m-3 mm-1)'
                
                nc_SNR=dataset.createVariable('SNR','f',ncShape2D)
                nc_SNR.long_name='Signal to noise ratio from signal without dealiasing '
                nc_SNR.units='dB'

                nc_Noi=dataset.createVariable('Noise','f',ncShape2D)
                nc_Noi.long_name='Noise from spectra reflectivity '
                nc_Noi.units='m-1'

                nc_nw=dataset.createVariable('Nw','f',ncShape2D)
                nc_nw.long_name='Intercept of the gamma distribution normalized to the Liquid Water Content calculated using only liquid hydrometeors according to hydrometeor type classification'
                nc_nw.units='log10(mm-1 m-3)'

                nc_dm=dataset.createVariable('Dm','f',ncShape2D)
                nc_dm.long_name='Mean mass-weighted raindrop diameter calculated using only liquid hydrometeors according to hydrometeor type classification'
                nc_dm.units='mm'


                nc_bb_bot=dataset.createVariable('BB_bottom','f',ncShape2D_BB)
                if np.isnan(h0_opt):
                    nc_bb_bot.long_name='Bright Band bottom height [m AGL]'
                else:
                    nc_bb_bot.long_name='Bright Band bottom height [m ASL]'
                            
                nc_bb_bot.units='m'

                nc_bb_top=dataset.createVariable('BB_top','f',ncShape2D_BB)
                if np.isnan(h0_opt):
                    nc_bb_top.long_name='Bright Band top height [m AGL]'
                else:
                    nc_bb_top.long_name='Bright Band top height [m ASL]'
    ...
    sweep_end_ray_index[:] = np.array([Timecount-1])
    f.close()
    if ~np.isnan(Dm_2).all() and ~np.isnan(Nw_2).all():
        ...
        nc_ranges_Dm.long_name = 'Mean diameter axes to rainfall type'
        nc_ranges_Dm.units = '(mm)'
        
        nc_ranges_Nw.long_name = 'Intecept parameter axes to Rainfall type'
        ...
        # Removed for WFIP3 - Particle ID gave questionable results in updrafts
        #nc_TypePrecipitation=dataset.createVariable('TyPrecipi','f',('Dm_ax','Nw_ax',))
        #nc_TypePrecipitation.long_name='Precipitation regime numerical value where possible values are: 5 (convective), 0 (transition) and -5 (stratiform) calculated using only liquid hydrometeors according to hydrometeor type classification'##, following the method of https://doi.org/10.1016/j.atmosres.2015.04.011'
